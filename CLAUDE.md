...
**Consigne is passive display only.** The optional `consigne` field (max 1000 chars) shows the teacher's exercise instruction as a read-only banner below the status bar. Settable via `.geomolo` file (or legacy `.tracevite`). The tool never validates, corrects, or interacts with the consigne — no exercise engine, no correction, no backend. Rendered via React JSX (auto-escaped) to prevent XSS. See spec.md §8.0.1.
